...
public class DBLogServiceImpl implements LogService {
    ...
    public void logRest(LogRestRecord logRestRecord) {
        ...
        try {
            ...
            int templateID =
                    logDaoService.getTemplateID(logRestRecord.getMethod(), logRestRecord.getTemplate());
            if (templateID == -1) {
                templateID =
                        logDaoService.insertTemplate(logRestRecord.getMethod(), logRestRecord.getTemplate());
            }
    ...
    public int searchCount(
            long igID, String user, String service, String activity, Date fromDate, Date toDate) {
        Integer result = 0;
        try {
    ...
    public List<LogSearchResultDAO> searchPage(
        ...
        try {
            logSearchResult =
                    logDaoService.searchPage(
                            igID, user, service, activity, fromDate, toDate, startRecord, pageSize);
        } catch (Exception e) {
            if (logger.isErrorEnabled()) {
                logger.error("Error searching log :", e);
            }
    ...
    public long countHistory(long itemID) {
        long count = 0;
        try {
            count = logDaoService.countHistory(itemID);
        } catch (Exception e) {
            if (logger.isErrorEnabled()) {
                logger.error("Error getting history for id: " + itemID, e);
            }
    ...
    public List<LogSearchResultDAO> getHistory(long itemID, long startRecord, long pageSize) {
        List<LogSearchResultDAO> logSearchResult = new ArrayList<>();
        try {
            logSearchResult = logDaoService.getHistory(itemID, startRecord, pageSize);
        } catch (Exception e) {
            if (logger.isErrorEnabled()) {
                logger.error("Error getting history for id: " + itemID, e);
            }
            ...
            try {
                ...
                if (activityID == -1) {
                    ...
                    if (service.matches(ALPHABETIC_SPACE_REGEX) && activity.matches(ALPHABETIC_SPACE_REGEX)) {
                        ...
                        if (logger.isErrorEnabled()) {
                            logger.error(
                                    "Service : " + service + " and activity : " + activity + " do not exists");
    ...
    public List<ActivityCountDAO> getListOfActivityCountForInterestGroup(Long igDbNode) {
        ...
        try {
            ...
            if (logger.isErrorEnabled()) {
                logger.error(
                        "Error during getting number of activity in log service for group id:" + igDbNode, e);
            }
        ...
        try {
            ...
            if (nodeRef == null) {
                ...
                if (logRestDAO.getNodeParent() != null) {
                    setConteinerData(dbLogRecord, new NodeRef(logRestDAO.getNodeParent()));
                } else {
                    setConteinerData(dbLogRecord, managementService.getCircabcNodeRef());
                }
                ...
                setConteinerData(dbLogRecord, nodeRef);
            }
            ...
            if (logger.isErrorEnabled()) {
                logger.error("Error when proccessing row " + logRestDAO.toString(), e);
            }
    ...
    private void setConteinerData(LogRecordDAO dbLogRecord, NodeRef nodeRef) {
