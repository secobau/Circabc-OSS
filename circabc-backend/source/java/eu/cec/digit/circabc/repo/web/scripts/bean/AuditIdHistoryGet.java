    ...
    protected Map<String, Object> executeImpl(WebScriptRequest req, Status status, Cache cache) {
        ...
        try {
            ...
            if (limit == 0) {
                ...
                results = this.logService.getHistory(documentId);

                historyCount = results.size();
            } else {
                historyCount = this.logService.countHistory(documentId);

                long startRecord = (page - 1) * limit;

                results = this.logService.getHistory(documentId, startRecord, limit);
            }
