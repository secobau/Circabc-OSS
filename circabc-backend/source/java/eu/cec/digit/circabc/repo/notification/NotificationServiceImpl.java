...
public class NotificationServiceImpl implements NotificationService {
    ...
    public void notify(NodeRef nodeRef, Set<NotifiableUser> users, NotificationType notificationType, String notificationText)
            throws Exception {
        ...
        if (nodeRef != null) {
            for (NotifiableUser nUser : users) {
                if (notificationType.equals(NotificationType.NOTIFY_USER_INVITATION)) {
                    ...
                    notifyImplInviteUser(nodeRef, nUser, notificationText);
                } else if (notificationType.equals(NotificationType.NOTIFY_USER_MEMBERSHIP_UPDATE)) {
                    notifyImplUpdateUser(nodeRef, nUser);
                }
            }
        }
    }
     ...
     * @throws MessagingException
     */
    private void notifyImplUpdateUser(NodeRef nodeRef, NotifiableUser nUser)
            throws MessagingException {
        String email = nUser.getEmailAddress();
        Locale userLocale = nUser.getNotificationLanguage();
        String igTitle = null;
