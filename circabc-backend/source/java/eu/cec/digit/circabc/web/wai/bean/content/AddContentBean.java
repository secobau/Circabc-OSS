...
public class AddContentBean extends BaseWaiDialog {
  ...
  public void init(final Map<String, String> parameters) {
    ...
    possibleStatus.add(0,
        new SelectItem(DocumentModel.STATUS_VALUE_DRAFT, translate(DocumentModel.STATUS_VALUE_DRAFT.toLowerCase())));
    possibleStatus.add(1,
        new SelectItem(DocumentModel.STATUS_VALUE_FINAL, translate(DocumentModel.STATUS_VALUE_FINAL.toLowerCase())));
    ...
    possibleSecurityRanking.add(0, new SelectItem(DocumentModel.SECURITY_RANKINGS_PUBLIC,
        translate(DocumentModel.SECURITY_RANKINGS_PUBLIC.toLowerCase())));
    possibleSecurityRanking.add(1, new SelectItem(DocumentModel.SECURITY_RANKINGS_INTERNAL,
        translate(DocumentModel.SECURITY_RANKINGS_INTERNAL.toLowerCase())));
    possibleSecurityRanking.add(2, new SelectItem(DocumentModel.SECURITY_RANKINGS_LIMITED,
        translate(DocumentModel.SECURITY_RANKINGS_LIMITED.toLowerCase())));

    possibleKeywords = new ArrayList<>();
    List<Keyword> lKeyword = keywordsService
        .getKeywords(getManagementService().getCurrentInterestGroup(this.getActionNode().getNodeRef()));
    for (Keyword keyword : lKeyword) {
      if (keyword.getMLValues().containsKey(fc.getViewRoot().getLocale())) {
        possibleKeywords
            .add(new SelectItem(keyword.getId().toString(), keyword.getMLValues().get(fc.getViewRoot().getLocale())));
      } else {
        possibleKeywords.add(new SelectItem(keyword.getId().toString(), keyword.getMLValues().toString()));
      }
    }
    ...
    if (uploadedFiles == null || uploadedFiles.size() == 0) {
      ...
      if (translationsAreValid()) {
        ...
        try {
          ...
          return outcome + CircabcNavigationHandler.OUTCOME_SEPARATOR + CircabcBrowseBean.PREFIXED_WAI_BROWSE_OUTCOME;
    ...
    for (CircabcUploadedFile file : uploadedFiles) {
      if (file.getIsTranslation()) {
        ...
        if (file.getLanguageName().equals(pivot.getLanguageName())) {
          Utils.addErrorMessage(translate("add_content_translation_pivot_same_language", file.getFileName(),
        ...
        for (String key : file.getSubmitedProperties().keySet()) {
          if (key.startsWith(":")) {
            if (usedLanguages.containsKey(file.getSubmitedProperty(key))) {
              Utils.addErrorMessage(translate("add_content_translations_same_language", key.substring(1),
                  usedLanguages.get(file.getSubmitedProperty(key)), getFilterLanguage(file.getSubmitedProperty(key))));
              result = false;
            } else {
  ...
  public void uploadFile(ActionEvent event) {
    if (submittedFile != null) {
      ...
      if (CircabcConfig.ECHA) {
        final List<String> allowedFileExtensions = ESAPI.securityConfiguration().getAllowedFileExtensions();
        try {
          ...
          Utils.addStatusMessage(FacesMessage.SEVERITY_WARN, translate(MSG_INVALID_FILE, allowedFileExtensions));
      ...
      if (!failed) {
        ...
        try {
          ...
          String encoding = metadataManager
              .guessEncoding(new BufferedInputStream(new FileInputStream(newFile.getFile())), mimetype);
        ...
        if (props != null) {
          if (props.containsKey(ContentModel.PROP_AUTHOR)) {
            newFile.addSubmitedProperty(CircabcUploadedFile.AUTHOR, props.get(ContentModel.PROP_AUTHOR).toString());
          }
  ...
  public void valiteFileSize(FacesContext context, UIComponent component, Object parameters) {
    ...
    if (requestContext.contentLength() > this.getFileUploadLimit().getMaxSizeInMegaBytes() * 1024 * 1024) {
    ...
    if (!createdNodeRefs.containsValue(bean.getFileName())) {
      ...
      if (bean.getSubmitedProperties().containsKey(CircabcUploadedFile.NAME)) {
        if (bean.getSubmitedProperties().get(CircabcUploadedFile.NAME).length() > 0) {
          createdRef = getContentBusinessSrv().addContent(parent, bean.getSubmitedProperty(CircabcUploadedFile.NAME),
              bean.getFile(), notificationDisabled);
        } else {
          createdRef = getContentBusinessSrv().addContent(parent, bean.getFileName(), bean.getFile(),
      ...
      // super.getNodeService().setProperty(createdRef,
      // ContentModel.PROP_VERSION_LABEL, "1.0");
      ...
      if (bean.getSubmitedProperties().containsKey(CircabcUploadedFile.DESCRIPTION)) {
        if (bean.getSubmitedProperties().get(CircabcUploadedFile.DESCRIPTION).length() > 0) {
          String tmpDescription = getSecurityService()
              .getCleanHTML(bean.getSubmitedProperties().get(CircabcUploadedFile.DESCRIPTION), true);

          super.getNodeService().setProperty(createdRef, ContentModel.PROP_DESCRIPTION, tmpDescription);
        }
      }
      ...
      if (bean.getIssueDate() != null) {
        super.getNodeService().setProperty(createdRef, DocumentModel.PROP_ISSUE_DATE, bean.getIssueDate());
      }
      if (bean.getExpirationDate() != null) {
        super.getNodeService().setProperty(createdRef, DocumentModel.PROP_EXPIRATION_DATE, bean.getExpirationDate());
      }
      ...
      if (bean.getIsPivotMultilingual()) {
         ...
         */
        getMultilingualContentService().addTranslation(createdRef,
            getPivotRef(bean.getSubmitedProperty(CircabcUploadedFile.IS_TRANSLATION_OF_DOCUMENT_TITLE)),
  ...
  public void addFile(final CircabcUploadedFile fileBean) {
    if (fileBean != null && fileBean.getFile() != null) {
      String fileName;
      try {
        fileName = getPropertiesBusinessSrv().computeValidUniqueName(getActionNode().getNodeRef(),
  ...
  /**
   * If the file was uploaded successfully, we open the edit content properties
   * dialog
   *
  ...
  protected String doPostCommitProcessing(final FacesContext context, final String outcome) {
    ...
    if (toEditPropsNode != null) {
      ...
      // as we were successful, go to the set properties dialog and set the dialog
      // parameters
      ...
      // We don't close the dialog cause the upload process clear the stack but we
      // fill it with the good thing
      FacesContext.getCurrentInstance().getViewRoot().setViewId(this.locationBeforeUpload);
      newOutcome = CircabcNavigationHandler.CLOSE_DIALOG_OUTCOME + CircabcNavigationHandler.OUTCOME_SEPARATOR
  ...
  public final void setShowOtherProperties(boolean ignore) {
    // do nothing, the value is submited in upload for (see
    // UploadedFile.addSubmitedProperty(edit-properties, value)
  }
  ...
  public final void setDisableNotification(boolean ignore) {
    // do nothing, the value is submited in upload for (see
    // UploadedFile.addSubmitedProperty(check-disable-notif, value)
  }
  ...
  public boolean isHasPermission() {
    ...
    final AccessStatus hasPermission = getPermissionService().hasPermission(currentNode.getNodeRef(), permission);
    if (hasPermission == AccessStatus.DENIED) {
      if (logger.isErrorEnabled()) {
        logger.error("User " + currentAlfrescoUserName + " does not have requred permision " + permission + " on node "
  ...
  public boolean isEncryptedPropertiesEnabled() {
    return Boolean.parseBoolean(CircabcConfiguration.getProperty(CircabcConfiguration.ENCRYPTED_PROPERTIES_ENABLED));
  }
}
