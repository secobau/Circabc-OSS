...
public class RestContentUpdateServlet extends BaseServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        final PermissionService permissionService = getServiceRegistry(getServletContext())
                .getPermissionService();

        final NodeService nodeService = getServiceRegistry(getServletContext()).getNodeService();

        circabcRenditionService = getCircabcServiceRegistry(getServletContext())
        ...
        if (permissionService.hasPermission(targetRef, PermissionService.WRITE_CONTENT)
                .equals(AccessStatus.DENIED)) {
            resp.sendError(HttpServletResponse.SC_FORBIDDEN, "Access denied - cannot update file");
            return;
        }
        ...
        try {
            ...
            if (fileItems.size() != 1) {
                resp.sendError(HttpServletResponse.SC_BAD_REQUEST, "Too many files in the request");
                return;
            }
            ...
            if (!item.isFormField()) {
                ...
                if (filename != null && filename.length() != 0) {
                    Map<String, String> res = handleUpdate(bean, item, targetRef);
     ...
     * @throws Exception
     */
    private Map<String, String> handleUpdate(FileUploadBean bean, final FileItem item,
        ...
        // streams.
        final TransactionService transactionService = getServiceRegistry(getServletContext())
                .getTransactionService();
        RetryingTransactionHelper txnHelper = transactionService.getRetryingTransactionHelper();
        final RetryingTransactionCallback<Map<String, String>> callback = new RetryingTransactionCallback<Map<String, String>>() {
                ...
                RunAsWork<Map<String, String>> work = new RunAsWork<Map<String, String>>() {
                    ...
                    public Map<String, String> doWork() throws Exception {
                        ...
                        if (item.getSize() > 0) {
                            ...
                            String newfilename = generateUniqueFilename(targetRef, filename);

                            final NodeService nodeService = getServiceRegistry(getServletContext())
                                    .getNodeService();
                            final MimetypeService mimetypeService = getServiceRegistry(getServletContext())
                                    .getMimetypeService();

                            Map<QName, Serializable> props = new HashMap<>();
                            props.put(ContentModel.PROP_NAME, newfilename);

                            nodeService.setProperty(targetRef, ContentModel.PROP_NAME, newfilename);
                            ...
                            final ContentService contentService = getServiceRegistry(getServletContext())
                                    .getContentService();
                            final ContentWriter writer = contentService
                                    .getWriter(targetRef, ContentModel.PROP_CONTENT,
                                            true);
                            writer.setMimetype(guessMimetype(filename));
                            writer.putContent(item.getInputStream());

                            final LogRecord logRecord = logUpdateRequest(targetRef, originalName);
                            ...
                            if (logger.isWarnEnabled()) {
                                logger
                                        .warn("Ignored file '" + filename + "' as there was no content, this is either "
        ...
        try {
            ...
            if (currentInterestGroup != null) {
                ...
                final Set<QName> aspects = nodeService
                        .getAspects(nodeService.getPrimaryParent(nodeRef).getParentRef());
                ...
                if (service != null) {
                    ...
                    logRecord.setInfo(UPDATE_FILE + " with original name" + originalFileName);
