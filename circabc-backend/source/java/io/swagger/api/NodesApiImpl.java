...
public class NodesApiImpl implements NodesApi {
    ...
    public static final QName PROP_DESTINATION =
            QName.createQName(NamespaceService.CONTENT_MODEL_1_0_URI, "destination");
	...
	private final List<String> NO_ACCESS_STRINGS = Arrays.asList(DirectoryPermissions.DIRNOACCESS.toString(),
    ...
    public Node getNode(NodeRef nodeRef, Node node) {
        ...
        if (secureNodeService.hasAspect(nodeRef, CircabcModel.ASPECT_INFORMATION)) {
            ...
            if (libRef != null) {
                isLibRootAdmin =
                        permissionService.hasPermission(libRef, "LibAdmin").equals(AccessStatus.ALLOWED);
            }
            ...
            if (newsRef != null) {
                isNewsRootAdmin =
                        permissionService.hasPermission(newsRef, "NwsAdmin").equals(AccessStatus.ALLOWED);
            }
        }
        ...
        for (Entry<QName, Serializable> property :
                secureNodeService.getProperties(nodeRef).entrySet()) {
        ...
        if (secureNodeService.getType(nodeRef).equals(ContentModel.TYPE_CONTENT)) {
            ContentData cData =
                    (ContentData) secureNodeService.getProperty(nodeRef, ContentModel.PROP_CONTENT);
            long size;
            if (cData != null) {
                size = cData.getSize();
            ...
            boolean isMultilingual =
                    secureNodeService.hasAspect(nodeRef, ContentModel.ASPECT_MULTILINGUAL_DOCUMENT);
            ...
            boolean isWorkingCopy =
                    secureNodeService.hasAspect(nodeRef, ContentModel.ASPECT_WORKING_COPY);
        ...
        for (org.alfresco.service.cmr.security.AccessPermission ac :
			...
			if(!hasGuestAccess && ac.getAuthority().equals("guest") && !NO_ACCESS_STRINGS.contains(permission)) {
        		hasGuestAccess = true;
        	}
            ...
            if (ac.getAuthorityType() == AuthorityType.USER) {
                if (ac.getAuthority().equals(userName)) {
                    ...
                    /*
                     * special case when dealing with the notifications, because DENIED prevails on ALLOWED
                     *
                     */
        ...
        if (secureNodeService.getType(nodeRef).equals(ContentModel.TYPE_FOLDER)) {
            ...
            Boolean hasFolders =
                    !nodeRef.toString().startsWith("versionStore")
                            && secureNodeService.getChildAssocs(nodeRef, typeFolder).size() > 0;
            ...
            Boolean hasFolders = secureNodeService.getChildAssocs(nodeRef, typeForum).size() > 0;
            node.setHasSubFolders(hasFolders);
        }
    ...
    @Override
    public SimpleId nodesIdGroupGet(String id) {
        NodeRef nodeRef = Converter.createNodeRefFromId(id);
        ...
        if (!(secureNodeService.hasAspect(nodeRef, CircabcModel.ASPECT_LIBRARY)
            ...
            throw new InvalidArgumentException(
                    "node must be part of a library / newsgroups or information service");
        }

        while (!secureNodeService.hasAspect(nodeRef, CircabcModel.ASPECT_IGROOT)) {
            nodeRef = secureNodeService.getPrimaryParent(nodeRef).getParentRef();
        }

        SimpleId simpleId = new SimpleId();
        simpleId.setId(nodeRef.getId());
    ...
    public String generateUniqueName(final NodeRef parent, final String candidateName) {
        //
        if (secureNodeService.getChildByName(parent, ContentModel.ASSOC_CONTAINS, candidateName)
                == null) {
            ...
            } while (secureNodeService.getChildByName(parent, ContentModel.ASSOC_CONTAINS, uniqueName)
                    != null);
