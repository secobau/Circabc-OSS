...
export class UserManagementComponent implements OnInit {
  ...
  public async searchUsers() {
    ...
    if (
      ...
      try {
        ...
        this.searchedUsers = await this.userService
          .getUsers(this.searchform.value.searchField)
          .toPromise();
      } catch (error) {
        console.error(error);
      }
  ...
  public async retrieveUsers() {
    try {
      if (this.uploadFiles.length > 0) {
        ...
        if (file) {
          this.retrievedUsers = await this.userService
            .getUsersFromList(file)
            .toPromise();
        }
      }
    } catch (error) {
  ...
  public async focusUser(userid: string) {
    ...
    if (userid !== '') {
      ...
      try {
        this.focusedUser = await this.userService
          .getUser(this.focusedUserId)
          .toPromise();
      } catch (error) {
        if (
          error.error.message &&
