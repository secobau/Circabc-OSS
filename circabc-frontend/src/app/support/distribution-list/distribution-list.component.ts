...
export class DistributionListComponent implements OnInit {
  ...
  public async loadDistributionEmails() {
    const distribs = await this.appMessageService
  ...
  private async changePage(listingOptions: ListingOptions) {
    ...
    await this.loadDistributionEmails();
  }
    ...
    if (distribMail.id) {
      try {
        await this.appMessageService
          .deleteDistributionEmails(distribMail.id)
          .toPromise();
        this.listingOptions = {
          page: 1,
          limit: 25,
    ...
    if (toRemove.length > 0) {
      ...
      for (const distrib of toRemove) {
        try {
          if (distrib.id) {
            await this.appMessageService
              .deleteDistributionEmails(distrib.id)
              .toPromise();
          }
        } catch (error) {
          console.error(error);
