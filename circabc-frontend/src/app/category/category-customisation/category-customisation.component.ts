...
export class CategoryCustomisationComponent implements OnInit {
  ...
  private async loadCategory(id: string) {
    ...
    this.loading = true;
    try {
      this.category = await this.nodesService.getNode(id).toPromise();
    } catch (error) {
  ...
  private async listLogos(id: string) {
    ...
    this.loading = true;
    try {
      this.logos = await this.categoryService
        .getCategoryLogoByCategoryId(id)
        .toPromise();
    } catch (error) {
    ...
    if (id && this.categoryId) {
      try {
        ...
        await this.categoryService
          .selectCategoryLogoByLogoId(this.categoryId, id)
          .toPromise();
  ...
  public async delete(id: string | undefined) {
    if (this.categoryId && id) {
      try {
        this.logos = await this.categoryService
          .deleteCategoryLogoByLogoId(this.categoryId, id)
          .toPromise();
      } catch (error) {
        console.error('impossible to select the image');
      }
