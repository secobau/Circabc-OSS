...
export class CreateGroupComponent implements OnInit {
  ...
  async createGroup() {
    ...
    if (this.categoryId) {
      try {
        ...
        const group = await this.categoryService
          .postInterestGroup(this.categoryId, newGroup)
          .toPromise();
        result.type = ActionType.CREATE_INTEREST_GROUP;
  ...
  async searchUsers() {
    if (!this.searchExpressionEmpty()) {
      ...
      this.availableUsers = await this.userService
        .getUsers(this.groupLeadersForm.controls.search.value)
        .toPromise();
