...
export class CategorySupportComponent implements OnInit {
  ...
  private async initCategory(id: string) {
    try {
      this.category = await this.categoryService.getCategory(id).toPromise();
  ...
  public async update() {
    ...
    if (this.category && this.category.id && this.isFormValid()) {
      try {
        this.category.useSingleContact = this.categoryForm.value.useSingleContact;
        ...
        await this.categoryService
          .putCategory(this.category.id, this.category)
          .toPromise();
        await this.initCategory(this.category.id);
      } catch (error) {
        console.error(error);
