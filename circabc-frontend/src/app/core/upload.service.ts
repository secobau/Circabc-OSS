...
export class UploadService {
  ...
  public async updateExistingFileContent(file: File, nodeId: string) {
  ...
  private async httpUpdateFile(file: File, destination: string) {
    return new Promise((resolve, reject) => {
      const ticket = this.loginService.getTicket();
      const url = `${this.serverURL}rest/update/${destination}`;
      this.sendRequest(url, ticket, file, resolve, reject);
    });
  }
