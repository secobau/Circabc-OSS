...
export class ValidationService {
  ...
  static getErrorMessageTranlationCode(code: string): string {
  ...
  static dateInfoValidator(control: AbstractControl) {
    ...
    let hour: number = Number(startTime.substring(0, 2));
    let minutes: number = Number(startTime.substring(3, 5));
  ...
  static phoneValidator(control: AbstractControl) {
    if (control.value === undefined || control.value === '') {
      return null;
    } else if (ValidationService.checkWholeStringNumber(control.value)) {
      return null;
    }
