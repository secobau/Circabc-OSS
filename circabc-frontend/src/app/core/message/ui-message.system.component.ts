...
export class UiMessageSystemComponent implements OnDestroy {
  ...
  public constructor(private uiMessageService: UiMessageService) {
    this.messageAnnouncedSubscription$ = this.uiMessageService.messageAnnounced$.subscribe(
      (message: UiMessage) => {
        this.messages.push(message);
      }
    );

    this.messageDestroyedSubscription$ = this.uiMessageService.messageDestroyed$.subscribe(
      (message: UiMessage) => {
