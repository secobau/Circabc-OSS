...
export class ForumBrowserComponent implements OnChanges, OnInit, OnDestroy {
  ...
  uiMessage = new UiMessage(
    ...
    this.translateService.instant('forums.message.publicly.available'),
    false,
    0
  );
  ...
  private subscribe() {
    this.actionFinishedSubscription$ = this.actionService.actionFinished$.subscribe(
  ...
  public async getCurrentForum() {
    ...
    if (this.forum.id) {
      this.path = await this.nodesService.getPath(this.forum.id).toPromise();

      this.content = await this.forumService
        .getForumContent(this.forum.id)
        .toPromise();
  ...
  public async refreshDeleteForum(res: ActionEmitterResult) {
    ...
    if (
      ...
      if (this.isForumRoot()) {
        this.forum = await this.nodesService
          .getNode(this.forum.id as string)
          .toPromise();
      } else {
        this.forum = await this.nodesService
          .getNode(this.forum.parentId as string)
          .toPromise();
      }
      await this.getCurrentForum();
  ...
  public async changeNotificationSubscription(value: string) {
    ...
    if (value && value !== '' && this.forum.id) {
      await this.notificationService
        ...
        .toPromise();
      this.forum = await this.nodesService.getNode(this.forum.id).toPromise();
    }
  }
