...
export class ViewEditDetailsForumComponent implements OnInit {
  ...
  private async fillForm() {
    try {
      this.forum = await this.nodesService.getNode(this.forumId).toPromise();
      ...
      if (this.forum.properties) {
        this.creator = await this.userService
          .getUser(this.forum.properties.creator)
          .toPromise();
        this.modifier = await this.userService
          .getUser(this.forum.properties.modifier)
          .toPromise();
      }
    } catch (error) {
  ...
  public async update() {
    try {
      ...
      if (this.forum !== undefined) {
        ...
        this.forum.description = this.updateForumForm.controls.description.value;
        this.forum.name = this.updateForumForm.controls.name.value;

        await this.forumService
          .putForum(this.forum.id as string, this.forum)
          .toPromise();
