...
export class PostComponent {
  ...
  public fireReply() {
    if (this.post && this.post.properties) {
      const allQuotesExpression = /<p><br><\/p><p>(\w)+: (((\w)+\s)*)(\w)+<\/p>(<p><br><\/p>)?<blockquote>(.)+<\/blockquote>$/;
      const message =
  ...
  public canEditComment(): boolean {
    return (
      this.permEvalService.isLibManageOwnOrHigher(this.post) ||
      this.permEvalService.isNewsgroupModerate(this.post) ||
      this.isOwner()
    );
  }
  ...
  public async approve() {
    if (this.post.properties) {
      await this.postService
        .putVerify(this.post.id as string, true)
        .toPromise();
  ...
  public async removeAbuses() {
    await this.postService.deleteAbuse(this.post.id as string).toPromise();
    const result: ActionEmitterResult = {};
    result.result = ActionResult.SUCCEED;
  ...
  private async getAbuses() {
    this.abuses = await this.postService
      .getAbuses(this.post.id as string)
      .toPromise();
  }
  ...
  public async accessAttachment(attachment: Attachment) {
    if (attachment.isLink) {
      const attachmentNode = await this.nodesService
        .getNode(attachment.id as string)
        .toPromise();
