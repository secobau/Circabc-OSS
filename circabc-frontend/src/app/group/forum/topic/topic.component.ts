...
export class TopicComponent implements OnInit {
  ...
  public async loadTopic(params: { [key: string]: string }) {
    ...
    if (params.nodeId !== undefined) {
      this.topicNode = await this.nodesService
    ...
    try {
      if (this.topicNode.id) {
        this.posts = await this.topicService
          .getReplies(
            this.topicNode.id,
  ...
  public async prepareQuote(post: Quote) {
    ...
    this.attachmentRemainingSize = await this.postService
  ...
  public async prepareEdit(post: ModelNode) {
    this.editPost = post;
    this.attachmentRemainingSize = await this.postService
      .getAttachmentsRemainingSize(this.editPost.id as string)
      .toPromise();
    if (post.properties) {
  ...
  public async refreshComments(res: ActionEmitterResult) {
    ...
    if (
      ...
      const txt = await this.translateService
        .get(getSuccessTranslation(ActionType.DELETE_POST))
        .toPromise();
  ...
  private async deleteAttachments() {
    if (this.editPost && this.editPost.id) {
      for (const attachment of this.attachmentsAndLinksToDelete) {
        await this.postService
          .deleteAttachment(this.editPost.id, attachment.id as string)
          .toPromise();
      }
    }
    this.attachmentsAndLinksToDelete = [];
  ...
  private async handleError(error: any) {
    ...
    if (
      ...
      res = await this.translateService
        .get('error.attachment.exceeds')
        .toPromise();
    } else if (error.error.message.includes('Invalid file type: ')) {
      res = await this.translateService
        .get('error.attachment.invalid', {
      ...
      res = await this.translateService.get('error.link.exists').toPromise();
    } else {
      res = await this.translateService.get('error.upload').toPromise();
    }
