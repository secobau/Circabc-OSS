...
export class ViewEditDetailsTopicComponent implements OnInit {
  ...
  private async fillForm() {
    this.topic = await this.nodesService.getNode(this.topicId).toPromise();
    ...
    if (this.topic.properties) {
      this.creator = await this.userService
        .getUser(this.topic.properties.creator)
        .toPromise();
      this.modifier = await this.userService
        .getUser(this.topic.properties.modifier)
        .toPromise();
    }
  }
  ...
  public async cancel() {
    ...
    // disable all form controls for view
    //    Object.keys(this.updateTopicForm.controls).forEach(key => {
    //      (this.updateTopicForm.get(key) as AbstractControl).disable();
    //    });
    this.goBack();
  }
  ...
  public async update() {
    try {
      ...
      if (this.topic !== undefined && this.topic.properties !== undefined) {
        ...
        this.topic.description = this.updateTopicForm.controls.description.value;
        this.topic.name = this.updateTopicForm.controls.name.value;
        this.topic.properties.security_ranking = this.updateTopicForm.controls.securityRanking.value;
        this.topic.properties.expiration_date = this.updateTopicForm.controls.expirationDate.value;

        await this.topicService
          .putTopic(this.topic.id as string, this.topic)
          .toPromise();
