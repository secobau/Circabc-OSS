...
export class AddConfigurationComponent implements OnInit {
  ...
  private async buildLibrarySectionTree() {
    ...
    if (this.ig && this.ig.libraryId) {
      ...
      this.path = await this.nodeService.getPath(this.ig.libraryId).toPromise();
    }
  }
  ...
  public async addConfiguration() {
    try {
      ...
      configuration.autoExtract = this.autoUploadForm.controls.autoExtractZip.value;
      configuration.jobNotifications = this.autoUploadForm.controls.jobNotifications.value;
      configuration.emails = this.autoUploadForm.controls.emailRecipients.value.replace(
        /\n/g,
        ','
      );
      ...
      await this.autoUploadService
        .postAutoUploadEntry(this.igId, configuration)
        .toPromise();
