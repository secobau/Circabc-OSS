...
export class AutoUploadComponent implements OnInit {
  ...
  private async getAutoUploadConfigurations() {
    const result: PagedAutoUploadConfiguration = await this.autoUploadService
    ...
    for (const configuration of this.configurations) {
      ...
      if (values.length === 0) {
        ...
        const day: string = await this.translateService
          .get(`cron-num-day.${values[1]}`)
          .toPromise();
        let hour: string;
        if (values[0] === '*') {
          hour = await this.translateService
            .get('label.every.hour.s')
            .toPromise();
        } else {
          hour = `${values[0]}:00`;
        }
