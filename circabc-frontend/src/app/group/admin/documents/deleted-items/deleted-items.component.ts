...
export class DeletedItemsComponent implements OnInit {
  ...
  private async loadIg() {
    ...
    if (this.groupId) {
      this.currentIg = await this.groupService
        ...
        .toPromise();
      this.deletedNodes = await this.archiveService
        .getDeletedDocuments(
          this.groupId,
  ...
  public async restoreFinish(res: ActionEmitterResult) {
    if (
      ...
      const txt = await this.translateService
        .get('admin.deleted.items.restore.succeed')
        .toPromise();
      ...
      const txt = await this.translateService
        .get('admin.deleted.items.restore.failed')
        .toPromise();
      this.uiMessageService.addSuccessMessage(txt);
    }
    ...
    try {
      if (archiveNode.id) {
        await this.archiveService
          .deleteDeletedDocument(this.currentIg.id as string, archiveNode.id)
          .toPromise();
        res.result = ActionResult.SUCCEED;
      }
  ...
  public async purgeFinish(res: ActionEmitterResult) {
    if (
      ...
      this.restorableNodes = [];
      const txt = await this.translateService
        .get('admin.deleted.items.purge.succeed')
        .toPromise();
      this.uiMessageService.addSuccessMessage(txt);
    } else if (
      res.type === ActionType.PURGE_CONTENT &&
      ...
      const txt = await this.translateService
        .get('admin.deleted.items.purge.failed')
        .toPromise();
