...
export class NotificationStatusComponent implements OnInit {
  ...
  private async listNotificationStatus(params: { [key: string]: string }) {
    ...
    this.currentNode = await this.nodesService.getNode(this.nodeId).toPromise();
    if (
  ...
  private async loadNotificationConfigurations() {
    ...
    if (this.nodeId !== undefined) {
      const result: PagedNotificationConfigurations = await this.notificationService
  ...
  public async deleteConfiguration(configuration: NotificationConfiguration) {
    await this.notificationService
      .deleteNotification(this.nodeId, configuration.authority as string)
      .toPromise();
    await this.loadNotificationConfigurations();
    await this.loadSubscribedUsers();
  }
  ...
  private async loadSubscribedUsers() {
    this.loading = true;
    if (this.nodeId !== undefined) {
      const result: PagedSubscribedUsers = await this.notificationService
        .getNotificationSubscribedUsers(
          this.nodeId,
