...
export interface PrettyProperty {
  name: string;
  value: string;
  icon: string;
}
export interface PrettyProperties {
  section: string;
  data: PrettyProperty[];
}
@Component({
...
export class AdminSummaryComponent implements OnInit {
  ...
  public async changeSummary() {
    this.selectedStatistics = this.summaryForm.controls.selectedStatistics.value;
  ...
  private async loadStatistics() {
    ...
    this.properties = await this.groupService
      .getIGSummaryStatistics(this.igId, false)
      .toPromise();
  ...
  public async calculateStatistics() {
    this.properties = await this.groupService
      .getIGSummaryStatistics(this.igId, true)
      .toPromise();
    this.prettifyProperties();
  }
  ...
  private async loadTimeline() {
    this.loading = true;
    this.toggleControls(false);
    this.activities = await this.groupService
      .getIGSummaryTimeline(this.igId)
      .toPromise();
    this.toggleControls(true);
    this.loading = false;
  }
  ...
  private async loadStructure() {
    this.loading = true;
    this.toggleControls(false);
    this.structureHolder = await this.groupService
      .getIGSummaryStructure(this.igId)
      .toPromise();
    this.toggleControls(true);
    this.loading = false;
  }
