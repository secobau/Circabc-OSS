...
export class AddPermissionsComponent implements OnInit {
  ...
  determineNodePermissionSet(): string[] {
    if (this.node.permissions && this.permissionCache.length === 0) {
      ...
      if (isInLibrary) {
        if (this.isContent()) {
          this.permissionCache = this.permEvalService.getLibraryContentPermissions();
        } else {
          this.permissionCache = this.permEvalService.getLibraryFolderPermissions();
        }
      } else {
  ...
  async addPermissions() {
    ...
    try {
      ...
      if (this.node.id) {
        this.perms = await this.permissionService
          .putPermission(this.node.id, body)
          .toPromise();
