...
export class ProfilesComponent implements OnInit {
  ...
  private async loadProfiles(params: { [key: string]: string }) {
    ...
    if (this.nodeId) {
      ...
      if (!this.currentGroup.isPublic) {
        const guestIdx = this.profiles.findIndex((value) => {
          return value.groupName === 'guest';
        });

        this.profiles.splice(guestIdx, 1);
      }

      if (!this.currentGroup.isRegistered) {
        const registeredIdx = this.profiles.findIndex((value) => {
          return value.groupName === 'GROUP_EVERYONE';
        });

        this.profiles.splice(registeredIdx, 1);
      }
    }
    this.loading = false;
  }
  ...
  public async export(profile: Profile) {
    profile.exported = true;
    if (profile.id) {
      await this.profileService.putProfile(profile.id, profile).toPromise();
    }
  }
    ...
    if (!this.loginService.isGuest()) {
      const userId =
        this.loginService.getUser().userId !== undefined
          ...
          : 'guest';
      if (userId !== undefined) {
        const currentUserMemberships = await this.userService
