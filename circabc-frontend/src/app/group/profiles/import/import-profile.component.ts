...
export class ImportProfileComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.igNodeId) {
      this.ig = await this.nodesService.getNode(this.igNodeId).toPromise();

      if (this.ig.parentId) {
        this.exportedProfiles = await this.categoryService
          .getExportedProfiles(this.ig.parentId, this.ig.id)
          .toPromise();
      }
    }
  }
      ...
      if (this.importProfileForm.value.selectedProfile === profile.id) {
        ...
        try {
          await this.profileService
            .postImportedProfile(this.igNodeId, body)
            .toPromise();
