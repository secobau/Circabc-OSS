...
export class AddKeywordComponent {
  ...
  public async loadAvailableKeywords() {
    ...
    if (this.groupNode.id && this.documentNode.id) {
      this.usedKeywords = await this.keywordsService
        .getKeywords(this.documentNode.id)
        .toPromise();
      const sourceKeywords = await this.keywordsService
        .getKeywordDefinitions(this.groupNode.id)
        .toPromise();
    ...
    for (const keyword of this.availableKeywords) {
      if (keyword.selected && this.documentNode.id) {
        await this.keywordsService
          .postKeyword(this.documentNode.id, keyword)
          .toPromise();
        keywordsAdded = true;
      }
    }
