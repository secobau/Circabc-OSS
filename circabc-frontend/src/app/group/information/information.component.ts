...
export class InformationComponent implements OnInit {
  ...
  public async loadInformation(params: { [key: string]: string }) {
    ...
    if (this.groupId && this.group.informationId) {
      this.informationNode = await this.nodesService
        .getNode(this.group.informationId)
        .toPromise();
      this.informationPage = await this.informationService
        .getInformationDefinitions(this.groupId)
        .toPromise();
      ...
      this.infoNews = await this.informationService
        .getInformationNews(
          this.groupId,
    ...
    if (value && value !== '' && this.informationNode.id) {
      try {
        await this.notificationService
          ...
          .toPromise();
        this.informationNode = await this.nodesService
          .getNode(this.informationNode.id)
          .toPromise();
      } catch (error) {
