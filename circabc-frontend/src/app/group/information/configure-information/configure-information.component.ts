...
export class ConfigureInformationComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    if (this.groupId) {
      this.infPage = await this.informationService
  ...
  public async save() {
    ...
    try {
      ...
      body.displayOldInformation = this.configurationForm.value.displayOldInformation;
      await this.informationService
        .putInformationDefinitions(this.groupId, body)
        .toPromise();
      this.showModal = false;
