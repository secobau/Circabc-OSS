...
export class AddNewsComponent implements OnInit {
  ...
  private async loadPageData(params: Params) {
    ...
    if (this.infoId) {
      this.infoNode = await this.nodeService.getNode(this.infoId).toPromise();
    }
  ...
  async loadEditableNews(id: string) {
    if (id !== undefined) {
      this.editableNews = await this.informationService
        .getNews(this.newsId)
        .toPromise();
  ...
  async saveNews() {
    ...
    try {
      ...
      const createdNews = await this.informationService
        .postInformationNews(this.groupId, news)
        .toPromise();
      ...
      if (
        ...
        if (this.filesToUpload.length === 1) {
          if (this.editableNews.files && this.editableNews.files.length > 0) {
            ...
            if (fileId) {
              await this.contentService.deleteContent(fileId).toPromise();
            }
          }
