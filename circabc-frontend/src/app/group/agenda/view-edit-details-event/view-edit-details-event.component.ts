...
export class ViewEditDetailsEventComponent implements OnInit {
  ...
  private async buildForm() {
    try {
      ...
      const eventDefinition = await this.eventsService
      ...
      this.updateEventForm = this.formBuilder.group(
        {
          ...
          meetingType: [eventDefinition.meetingType],
          dateInfo: this.formBuilder.group(
            ...
            { validator: ValidationService.timeInfoValidator }
          ),
      ...
      if (eventDefinition.igId !== undefined) {
        ...
        if (this.ig === undefined) {
          this.ig = await this.groupService
  ...
  private async buildLibrarySectionTree() {
    this.eventRootNode = await this.nodesService
      .getNode(this.ig.eventId as string)
      .toPromise();
    ...
    if (this.ig && this.ig.libraryId && this.libraryRoot === undefined) {
      ...
      if (
        ...
        this.path = await this.nodesService
          .getPath(this.updateEventForm.controls.meetingLibrarySection.value)
          .toPromise();
      }
    }
  }
    ...
    try {
      ...
      if (
        ...
        eventDefinition.attendantsInfo.invitedUsersOrProfiles = eventDefinition.attendantsInfo.invitedUsersOrProfiles
          ...
          .map((item: any) => (item.userId === undefined ? item : item.userId));
