...
export class ListComponent implements OnInit {
  ...
  public async changePage(listingOptions: ListingOptions) {
    ...
    if (this.igId !== undefined) {
      const result: PagedEventItemDefinition = await this.eventsService
        .getInterestGroupListEvents(
          this.igId,
          this.filter,
      ...
      if (this.eventRootNode === undefined) {
        const ig: InterestGroup = await this.interestGroupService
          .getInterestGroup(this.igId)
          .toPromise();

        this.eventRootNode = await this.nodesService
          .getNode(ig.eventId as string)
          .toPromise();
      }
    }
    this.loading = false;
