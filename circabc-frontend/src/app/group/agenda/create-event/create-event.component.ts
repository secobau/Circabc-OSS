...
export class CreateEventComponent implements OnInit, OnChanges {
  ...
  private async loadData() {
    if (this.ig === undefined) {
      this.ig = await this.interestGroupService
    ...
    this.newEventForm = this.formBuilder.group(
      {
        ...
        dateInfo: this.formBuilder.group(
          ...
          { validator: ValidationService.dateInfoValidator }
        ),
        language: ['en'],
        ...
        repeatsInfo: this.formBuilder.group(
          ...
          { validator: ValidationService.repeatsSelectedValidator }
        ),
        // second wizard step
        attendantsInfo: this.formBuilder.group(
          ...
          { validator: ValidationService.attendantsValidator }
        ),
  ...
  private async buildLibrarySectionTree() {
    if (this.ig && this.ig.libraryId) {
      this.path = await this.nodesService
        .getPath(this.ig.libraryId)
        .toPromise();
  ...
  public async save() {
    try {
      ...
      (this.newEventForm.controls
        .dateInfo as FormGroup).controls.date.patchValue(adaptedDate);
      ...
      if (
        ...
        eventDefinition.attendantsInfo.invitedUsersOrProfiles = eventDefinition.attendantsInfo.invitedUsersOrProfiles
        ...
        eventDefinition.repeatsInfo.mainOccurence = !this.newEventForm.value
          .repeatsInfo.repeats
          ? 'OnlyOnce'
          : this.newEventForm.value.repeatsInfo.whenOrEverySelect ===
            'whenSelect'
          ? 'Times'
          : 'EveryTimes';
        ...
        await this.eventsService
          .postEvent(this.igId, eventDefinition)
          .toPromise();
      }
