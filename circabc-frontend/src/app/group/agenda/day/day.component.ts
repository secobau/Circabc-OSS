...
export class DayComponent implements OnChanges {
  ...
  public async getEvents() {
    if (this.meMode) {
      this.events = await this.userService
        .getUserEventsPeriod(this.getUserId(), getFullDate(this.date), 'Exact')
        .toPromise();
    } else {
      this.events = await this.eventsService
            ...
            if (event.startTime !== undefined && event.endTime !== undefined) {
              ...
              if (
                ...
                if (amount === 1) {
                  let color:
                    | string
                    | undefined = this.agendaHelperService.getEventDisplayColor(
                    event.id as string
                  );
                  if (color === undefined) {
