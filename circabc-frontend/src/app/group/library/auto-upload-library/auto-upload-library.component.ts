...
export class AutoUploadLibraryComponent implements OnInit {
  ...
  private async loadConfiguration() {
    ...
    if (this.igId !== undefined && this.nodeId !== undefined) {
      this.configuration = await this.autoUploadService
        .getAutoUploadEntry(this.igId, this.nodeId)
        .toPromise();
  ...
  public async testConnection() {
    ...
    const result = await this.ftpService
      .testFTPConnectionOnServer(
  ...
  public async save(action: string) {
    ...
    try {
      ...
      this.configuration.ftpPath = this.autoUploadForm.controls.pathToFile.value;
      this.configuration.ftpUsername = this.autoUploadForm.controls.username.value;
      this.configuration.ftpPassword = this.autoUploadForm.controls.password.value;
      this.configuration.autoExtract = this.autoUploadForm.controls.autoExtractZip.value;
      this.configuration.jobNotifications = this.autoUploadForm.controls.jobNotifications.value;
      this.configuration.emails = this.autoUploadForm.controls.emailRecipients.value.replace(
        /\n/g,
        ','
      );
      ...
      await this.autoUploadService
        .postAutoUploadEntry(this.igId, this.configuration)
        .toPromise();
