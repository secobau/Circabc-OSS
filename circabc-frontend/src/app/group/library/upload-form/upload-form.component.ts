...
export class UploadFormComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    this.targetNode = await this.nodesService
      .getNode(this.targetNodeId)
      .toPromise();
  }
  ...
  private async uploadFilesOrPivot() {
    ...
    for (const fileUpload of this.filesToUpload) {
      if (
        ...
        try {
          const end = await this.fileService
            .uploadFile(
              this.targetNodeId,
              fileUpload.name,
              fileUpload.title,
  ...
  private async fireNotifications() {
    ...
    try {
      await this.fileService
        .fireNewContentNotification(this.targetNodeId, nodeRefs)
        .toPromise();
    } catch (error) {
      console.error(error);
    }
