...
export class ClipboardComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Output()
  public readonly actionFinished: EventEmitter<
    ActionEmitterResult
  > = new EventEmitter<ActionEmitterResult>();
  @Output()
  public readonly itemsAmount: EventEmitter<number> = new EventEmitter<
    number
  >();
  // clipboard contents
  public nodes: Node[] = [];
  ...
  private subscribe() {
    ...
    this.itemsRemovedSubscription$ = this.clipboardService.itemsRemoved$.subscribe(
      (node: Node) => {
        if (
          this.nodes.find((arrayNode) => arrayNode.id === node.id) !== undefined
        ) {
  ...
  public async copyPasteItem(node: Node) {
    ...
    if (node.id !== undefined && this.currentStandingNode.id !== undefined) {
      ...
      await this.nodesService
        .postPaste(this.currentStandingNode.id, nodeIds)
        .toPromise();
  ...
  public async cutPasteItem(node: Node) {
    ...
    if (node.id !== undefined && this.currentStandingNode.id !== undefined) {
      ...
      await this.nodesService
        .putPaste(this.currentStandingNode.id, nodeIds)
        .toPromise();
      this.removeItem(node);
      this.actionFinished.emit(
  ...
  public async linkPasteItem(node: Node) {
    ...
    if (node.id !== undefined && this.currentStandingNode.id !== undefined) {
      ...
      nodeIds.push(node.id);
      await this.nodesService
        .postLink(this.currentStandingNode.id, nodeIds)
        .toPromise();
      this.removeItem(node);
      this.actionFinished.emit(
  ...
  public async copyPasteAll() {
    ...
    if (this.currentStandingNode.id !== undefined) {
      ...
      await this.nodesService
        .postPaste(this.currentStandingNode.id, nodeIds as string[])
        .toPromise();
      this.actionFinished.emit(
        this.buildSuccessResult(ActionType.CLIPBOARD_COPY_NODE)
      );
  ...
  public async cutPasteAll() {
    ...
    if (this.currentStandingNode.id !== undefined) {
      const nodeIds = this.nodes.map((localNode) => localNode.id);
      await this.nodesService
        .putPaste(this.currentStandingNode.id, nodeIds as string[])
        .toPromise();
      this.removeAll();
      this.actionFinished.emit(
        this.buildSuccessResult(ActionType.CLIPBOARD_MOVE_NODE)
  ...
  public async linkPasteAll() {
    ...
    if (this.currentStandingNode.id !== undefined) {
      const nodeIds = this.nodes.map((localNode) => localNode.id);
      await this.nodesService
        .postLink(this.currentStandingNode.id, nodeIds as string[])
        .toPromise();
      this.removeAll();
      this.actionFinished.emit(
        this.buildSuccessResult(ActionType.CLIPBOARD_LINK_NODE)
