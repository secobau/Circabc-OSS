...
export class EditNodeComponent implements OnInit {
  ...
  public async loadDynamicPropertiesModel(groupId: string) {
    if (groupId) {
      this.dynamicPropertiesModel = await this.dynamicPropertiesService
        .getDynamicPropertyDefinitions(groupId)
        .toPromise();
  ...
  public async loadNode(nodeId: string) {
    this.node = await this.nodesService.getNode(nodeId).toPromise();
  ...
  public async updateProperties() {
    ...
    if (isFile) {
      await this.contentService
        .putContent(this.node.id as string, tmpNode)
        .toPromise();
    } else {
      await this.spaceService
        .putSpace(this.node.id as string, tmpNode)
        .toPromise();
    }
