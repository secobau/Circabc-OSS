...
export class MembersComponent implements OnInit {
  ...
  private async loadMembers(params: { [key: string]: string }) {
    ...
    if (this.nodeId !== undefined) {
      this.currentGroup = await this.groupService
        .getInterestGroup(this.nodeId)
        .toPromise();
      await this.changePage(this.listingOptions);
      this.availableProfiles = await this.profileService
        .getProfiles(this.nodeId)
        .toPromise();
    ...
    if (result !== undefined) {
      if (
        ...
        const res = await this.translateService
          .get(getSuccessTranslation(ActionType.ADD_MEMBERSHIPS))
          .toPromise();
  ...
  public async uninviteWizardClosed(result: ActionEmitterResult) {
    if (result !== undefined) {
      if (
        ...
        const res = await this.translateService
          .get(getSuccessTranslation(ActionType.REMOVE_MEMBERSHIPS))
          .toPromise();
        this.uiMessageService.addSuccessMessage(res, true);
        this.selectedUsers = [];
        ...
        const res = await this.translateService
          .get(getErrorTranslation(ActionType.REMOVE_MEMBERSHIPS))
          .toPromise();
        this.uiMessageService.addErrorMessage(res, true);
      }
    }
  ...
  public async changeModalClosed(res: ActionEmitterResult) {
    if (res.result === ActionResult.CANCELED) {
      ...
      const txt = await this.translateService
        .get(getSuccessTranslation(ActionType.CHANGE_PROFILE))
        .toPromise();
      this.uiMessageService.addSuccessMessage(txt, true);
      await this.changePage(this.listingOptions);
    } else if (res.result === ActionResult.FAILED) {
      const txt = await this.translateService
        .get(getErrorTranslation(ActionType.CHANGE_PROFILE))
        .toPromise();
  ...
  public async multipleChangeModalClosed(res: ActionEmitterResult) {
    if (res.result === ActionResult.CANCELED) {
      ...
      const txt = await this.translateService
        .get(getErrorTranslation(ActionType.CHANGE_PROFILES))
        .toPromise();
      this.uiMessageService.addErrorMessage(txt, false);
    }
  }
  ...
  public async expirationModalClosed(res: ActionEmitterResult) {
    if (res.result === ActionResult.CANCELED) {
      ...
      this.allSelected = false;
      const txt = await this.translateService
        .get(getSuccessTranslation(ActionType.EDIT_EXPIRATION))
        .toPromise();
      this.uiMessageService.addSuccessMessage(txt, true);
      this.refreshUsers();
    } else if (res.result === ActionResult.FAILED) {
      this.showExpirationDialog = false;
      const txt = await this.translateService
        .get(getErrorTranslation(ActionType.EDIT_EXPIRATION))
        .toPromise();
      this.uiMessageService.addErrorMessage(txt, false);
      this.refreshUsers();
    }
