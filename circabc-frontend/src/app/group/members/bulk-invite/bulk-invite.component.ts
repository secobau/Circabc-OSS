...
export class BulkInviteComponent implements OnInit {
  ...
  private async load() {
    ...
    this.availableCategories = await this.userService
      .getBulkInviteCategories(this.user.userId as string)
      .toPromise();
  ...
  private async getCurrentUserMemberships() {
    if (!this.loginService.isGuest()) {
      ...
      if (userId !== undefined) {
        this.currentUserMemberships = await this.userService
  ...
  private async loadGroup() {
    this.currentGroup = await this.interestGroupService
  ...
  private async loadIGs() {
    this.loading = true;
    const igs = await this.userService
      .getBulkInviteIGs(this.selectedCategory.id as string, this.igId)
      .toPromise();
  ...
  public async loadMembers() {
    ...
    if (
      ...
      this.allMembers = await this.userService
        .getBulkInviteMembers(
    ...
    if (this.fileToUpload !== undefined) {
      ...
      try {
        allMembersFromFile = await this.userService
          .bulkInviteUsersDigestFile(this.igId, this.fileToUpload)
          .toPromise();
        ...
        if (errPos > -1) {
          ...
          if (errorDetails.startsWith('NumCol<9:')) {
            ...
            result = await this.translateService
              .get('label.error.invalid.bulk.file.columns', {
                details: colNum,
              })
              .toPromise();
            ...
            if (match !== null) {
              ...
              const found = match[3];
              result = await this.translateService
                .get('label.error.invalid.bulk.file.nocolumn', {
                  name: name,
                  pos: pos,
                  found: found,
                })
            ...
            result = await this.translateService
              .get('label.error.invalid.bulk.file.nullcolumn', {
                details: colNum,
              })
              .toPromise();
          }
        ...
        if (result === '') {
          result = await this.translateService
            .get('label.error.invalid.bulk.file')
            .toPromise();
        }
    ...
    // remove unstable members (in error)
    const tempMembers: SelectableBulkImportUserData[] = [];
    for (const member of this.allMembers) {
      if (member.status === 'ok' || member.status === 'member') {
        tempMembers.push(member);
      }
    }
    this.allMembers = tempMembers;
    this.moveAccessToTop();
    // set new default profile value
    /*for (const member of this.allMembers) {
  ...
  public canInvite() {
    return this.allMembers !== undefined && this.allMembers.length > 0;
  }
