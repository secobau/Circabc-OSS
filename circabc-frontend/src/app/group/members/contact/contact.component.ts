...
export class ContactComponent implements OnInit {
  ...
  subscribeToCheckSaveTemplateChanges() {
    const checkSaveTemplateChanges = this.emailForm.controls.checkSaveTemplate
      .valueChanges;
  ...
  public async loadGroup(id: string) {
    this.currentIg = await this.interestGroupService
      .getInterestGroup(id)
      .toPromise();
  }

  public async loadMailTemplates() {
    this.templates = await this.emailService.getUserMailTemplates().toPromise();
  ...
  async sendEmail() {
    ...
    await this.emailService
      .postGroupEmail(this.currentIg.id as string, body)
      .toPromise();
    this.cancel();
  ...
  public async saveTemplate() {
    try {
      ...
      const res = await this.translateService.get(errMessage).toPromise();
  ...
  public async removeTemplate() {
    try {
      await this.emailService
        .deleteUserMailTemplates([this.selectedTemplateId])
        .toPromise();
      this.updateTemplateFields('0');

      const res = await this.translateService
        .get('label.template.removed')
        .toPromise();
      this.uiMessageService.addSuccessMessage(res, true);
    } catch (error) {
      const res = await this.translateService
        .get('label.template.removed.error')
        .toPromise();
      this.uiMessageService.addErrorMessage(res);
    }
    await this.loadMailTemplates();
