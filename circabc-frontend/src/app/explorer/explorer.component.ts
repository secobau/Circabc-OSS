...
export class ExplorerComponent implements OnInit {
  ...
  public async ngOnInit() {
    this.title.setTitle('Group Browser');
    try {
      this.preparing = true;
      const unsortedHeaders = await this.headerService
      ...
      if (headerFromQuery !== undefined && headerFromQuery !== '') {
        this.currentHeader = await this.headerService
          .getHeader(headerFromQuery)
          .toPromise();
      ...
      if (categoryFromQuery !== undefined && categoryFromQuery !== '') {
        this.currentCategory = await this.categoryService
          .getCategory(categoryFromQuery)
          .toPromise();
        this.step = 'groups';
      } else if (
  ...
  public async listCategories(header: Header | undefined) {
    ...
    try {
      if (this.currentHeader.id) {
        const unsortedCategories = await this.headerService
          .getCategoriesByHeaderId(this.currentHeader.id, this.getCurrentLang())
          .toPromise();
  ...
  public async listInterestGroups(category: ModelNode | undefined) {
    ...
    try {
      if (category.id) {
        this.currentCategory = await this.nodesService
          .getNode(category.id)
          .toPromise();

        const unsortedInterestGroups = await this.categoryService
          .getInterestGroupsByCategoryId(category.id, this.getCurrentLang())
          .toPromise();
