...
export class AddLinkComponent implements OnInit, OnChanges {
  ...
  async ngOnChanges(changes: SimpleChanges) {
    if (changes.linkId) {
      if (changes.linkId.currentValue) {
        this.linkToEdit = await this.helpService
          .getHelpLink(changes.linkId.currentValue)
          .toPromise();
  ...
  public async updateLink() {
    ...
    try {
      if (this.linkId) {
        ...
        await this.helpService.updateHelpLink(body.id, body).toPromise();
  ...
  public async createLink() {
    ...
    try {
      await this.helpService.createHelpLink(this.newLinkForm.value).toPromise();
      this.linkCreated.emit();
      this.newLinkForm.reset();
    } catch (error) {
      console.error(error);
