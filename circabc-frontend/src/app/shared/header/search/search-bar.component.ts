...
export class SearchBarComponent implements OnInit {
  ...
  private buildForm(): void {
    ...
    // tslint:disable-next-line:promise-function-async no-floating-promises
    this.searchForm.valueChanges
      .pipe(debounceTime(debounceTimeInMiliSecond))
      .subscribe((data) => this.onValueChanged(data)); // tslint:disable-line
    this.clearResults();
  }
    ...
    if (data.searchString !== '' && !this.searching && this.groupId) {
      ...
      try {
        const res = await this.searchService
          .getSearch(data.searchString, this.groupId)
          .toPromise();
        if (res.data) {
