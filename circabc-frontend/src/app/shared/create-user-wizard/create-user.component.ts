...
export class CreateUserComponent implements OnInit, OnChanges {
  ...
  public async submitNewUser() {
    ...
    try {
      await this.userService.postUser(postData).toPromise();
      ...
      const res = await this.translateService
        .get(getSuccessTranslation(ActionType.CREATE_USER))
        .toPromise();
      ...
      const res = await this.translateService.get(messageKey).toPromise();
