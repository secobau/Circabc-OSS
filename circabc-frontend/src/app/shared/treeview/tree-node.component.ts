...
export class TreeNodeComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit() {
    await this.reload();
    this.langaugeChangeSubscription$ = this.translateService.onLangChange.subscribe(
      async (_event: LangChangeEvent) => {
        await this.reload();
      }
    );
  }
  ...
  private async listForums() {
    const subForums = await this.forumService
      .getSubforums(this.node.nodeId, 'title', 'ASC')
      .toPromise();
  ...
  private async listSpaces() {
    const subspaces = await this.spaceService
      .getSubspaces(this.node.nodeId, '', 'title', 'ASC')
      .toPromise();
