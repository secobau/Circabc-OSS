...
export class UsersPickerComponent implements OnInit, ControlValueAccessor {
  ...
  private async buildCompleteUsersArray(
    ...
    try {
      const pagedUserProfile: PagedUserProfile = await this.membersService
        .getMembers(
          this.igId,
          undefined,
          undefined,
      ...
      //      this.uiMessageService.addErrorMessage(error);
    }
  }
    ...
    try {
      if (this.igId) {
        ...
        if (this.selectedTypeValue === '0') {
          ...
          let profiles: Profile[] = await this.profileService
            .getProfiles(this.igId, undefined, this.searchText, false)
            .toPromise();
